...
export default function SpicyPage() {
  ...
  // 점수판은 좌석 순서로 고정한다. 점수순으로 정렬하면 점수가 바뀔 때마다 줄이 뒤바뀌어
  // "지금 누구 차례고 다음이 누구인지" 읽을 수가 없다(건의사항). 순위는 뱃지로 보여준다.
  const bySeat = ss ? [...ss.players].filter((p) => !p.left).sort((a, b) => a.seat - b.seat) : [];
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-[1fr_320px] xl:grid-cols-[1fr_380px] gap-3 sm:gap-4 lg:gap-5 items-start lg:items-stretch lg:min-h-[calc(100vh-150px)]">
            ...
            <div className="flex items-center justify-between text-xs sm:text-sm lg:text-base flex-none">
              ...
              <span className="font-extrabold text-rose-600">
                {ended ? '게임 종료'
                  ...
                  : `${ss.turnName ?? ''}님 차례…`}
                {!ended && ss.turnPhase === 'PLAY' && ss.nextSeat >= 0 && ss.nextSeat !== ss.turnSeat && (
                  <span className="ml-2 font-normal text-[11px] lg:text-xs text-slate-400">
                    ▸ 다음 {seatName(ss.nextSeat)}
                  </span>
                )}
              </span>
            </div>
          ...
          <div className="flex flex-col gap-3 sm:gap-4 min-h-0">
            <div className="rounded-2xl border-2 border-slate-200 overflow-hidden bg-white flex-none">
              <p className="px-3 py-2 lg:py-2.5 text-xs lg:text-sm font-extrabold text-slate-400 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                <span>점수판</span>
                <span className="font-normal text-[10px] lg:text-[11px] text-slate-300">위에서 아래로 차례가 돌아요</span>
              </p>
              {bySeat.map((p) => {
                const isTurn = p.seat === ss.turnSeat && !ended;
                const isNext = p.seat === ss.nextSeat && !ended && !isTurn;
                return (
                <div key={p.seat} className={`px-3 py-2 lg:py-2.5 border-b border-slate-50 last:border-b-0 ${isTurn ? 'bg-rose-50 border-l-4 border-l-rose-400' : isNext ? 'bg-slate-50/70' : ''}`}>
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-sm lg:text-base flex items-center gap-1.5 min-w-0">
                      <span className={`w-4 flex-none text-center text-xs ${isTurn ? 'text-rose-500' : 'text-slate-300'}`}>
                        {isTurn ? '▶' : isNext ? '⌄' : ''}
                      </span>
                    ...
                    <span className="flex items-center gap-1.5 flex-none">
                      <span className={`text-[10px] lg:text-[11px] font-bold px-1.5 py-0.5 rounded-full ${rankOf(p.seat) === 1 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-400'}`}>
                        {rankOf(p.seat)}위
                      </span>
