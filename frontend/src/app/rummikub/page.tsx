  ...
  if (!roomCode) {
    return (
      <main className="min-h-screen flex flex-col items-center p-6 max-w-lg mx-auto w-full">
        ...
        {adminFooter}
      <RoomChat game="rummikub" roomCode="lobby" clientId={clientId} nick={nick} />
      </main>
    );
  }
    ...
    <main className={`min-h-screen flex flex-col items-center p-4 ${wide ? 'max-w-5xl' : 'max-w-lg'} mx-auto w-full`}>
      <div className="w-full flex items-center justify-between gap-2 flex-wrap mb-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <button onClick={() => { api(`/api/v1/rummikub/leave?roomCode=${roomCode}&clientId=${cid()}`, { method: 'POST' }).catch(() => {}); changeRoom(null); setSt(null); }} className="text-xs text-gray-400 underline shrink-0">나가기</button>
        </div>
