...
/**
 * 사주 해석용 OpenAI Chat Completions 클라이언트 (JSON 응답 전용).
 *
 * <p>API 키({@code app.openai.api-key})는 임베딩·마피아 봇과 공유하지만,
 * 모델·타임아웃 등은 {@code app.saju.chat-*} 로 따로 둔다. 마피아 봇
 * ({@code com.wordplay.mafia.ai.OpenAiChatClient})과 같은 키를 쓰면 한쪽 설정을
 * 바꿀 때 다른 쪽까지 끌려간다.
 *
 * <p>엔드포인트를 {@code app.saju.chat-url} 로 바꿀 수 있으므로 OpenAI 호환 API라면
 * 다른 게이트웨이로도 붙일 수 있다.
 *
...
public class SajuAiClient {
    ...
    @Value("${app.saju.chat-url:https://api.openai.com/v1/chat/completions}")
    private String url;

    @Value("${app.saju.chat-model:gpt-4o-mini}")
    private String model;

    @Value("${app.openai.api-key:}")
    private String apiKey;

    @Value("${app.saju.chat-timeout-ms:60000}")
