...
public class MonopolyController {
    ...
    private final MonopolyRoomManager rooms;
    ...
    @PostMapping("/new")
    public ApiResponse<CreateRoomResponse<MonopolyState>> newGame(@RequestParam String clientId,
                                                                  @RequestBody NewMonopolyRequest req) {
    ...
    @PostMapping("/join")
    public ApiResponse<MonopolyState> join(@RequestParam String roomCode, @RequestParam String clientId,
                                           @RequestBody JoinRequest req) {
        validateClientId(clientId);
        MonopolyGame g = rooms.require(roomCode);
        g.join(clientId, req.nick());
        return ApiResponse.success(g.me(clientId));
    }

    @PostMapping("/add-bot")
    public ApiResponse<MonopolyState> addBot(@RequestParam String roomCode, @RequestParam String clientId,
    ...
    @PostMapping("/start")
    public ApiResponse<MonopolyState> start(@RequestParam String roomCode, @RequestParam String clientId) {
        validateClientId(clientId);
        MonopolyGame g = rooms.require(roomCode);
        g.start(clientId);
        return ApiResponse.success(g.me(clientId));
    ...
    @PostMapping("/roll")
    public ApiResponse<MonopolyState> roll(@RequestParam String roomCode, @RequestParam String clientId,
                                           @RequestParam(defaultValue = "60") int power) {
        validateClientId(clientId);
        MonopolyGame g = rooms.require(roomCode);
        g.roll(clientId, power);
        return ApiResponse.success(g.me(clientId));
    }

    @PostMapping("/decide")
    public ApiResponse<MonopolyState> decide(@RequestParam String roomCode, @RequestParam String clientId,
                                             @RequestParam String action, @RequestParam(defaultValue = "-1") int arg) {
        validateClientId(clientId);
        MonopolyGame g = rooms.require(roomCode);
        g.decide(clientId, action, arg);
        return ApiResponse.success(g.me(clientId));
    }

    @GetMapping("/me")
    public ApiResponse<MonopolyState> me(@RequestParam String roomCode, @RequestParam String clientId) {
        validateClientId(clientId);
        MonopolyGame g = rooms.find(roomCode);
        if (g == null) return ApiResponse.success(MonopolyState.notFound(System.currentTimeMillis()));
        return ApiResponse.success(g.me(clientId));
    }
