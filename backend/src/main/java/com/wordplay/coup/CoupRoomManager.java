...
public class CoupRoomManager {
    ...
    public String create(String clientId, NewCoupRequest req) {
        ...
        game.newGame(clientId, req.nick(), req.reactionSec());
        try {
            return reg.add(game);
    ...
    public CoupGame require(String code) {
        CoupGame g = reg.find(code);
        if (g == null) throw new BusinessException(ErrorCode.INVALID_INPUT, "방을 찾을 수 없습니다");
        return g;
    }

    public CoupGame find(String code) { return reg.find(code); }
